...
setSeeSearchBar(prev=>!prev)
  ...
  return (
    <div
      className={`fixed z-30 top-0 left-0 right-0 px-4 sm:px-6 md:px-7 lg:px-8 xl:px-40 w-full h-16  flex flex-row md:justify-center justify-between items-center  transition-colors duration-300
        ${scrolled ? "bg-gray-100/50  backdrop-blur-md shadow-md" : "bg-orange-50"}
        `}
    >
      {/* ---logo --- */}
...
<span className='font-bold text-gray-100 md:text-sm text-[10px] '>{cartCount}</span>
        ...
        {/* heart Icon */}
        <Svg svgId="heart" className="md:w-7 md:h-7 text-amber-900  cursor-pointer   duration-300
