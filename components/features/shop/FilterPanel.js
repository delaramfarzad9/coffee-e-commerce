 ...
 */
const FilterPanel = ({ closePanel, onSortChange, onFiltersChange, currentFilters,productCount }) => {
    ...
    <div className="fixed inset-0 z-40 ">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <Svg svgId="close" onClick={closePanel} className="cursor-pointer w-8 h-8" />
        </div>

        <p className="mt-2  border-b pb-2 border-b-chocolate/30 text-chocolate/80 text-lg font-medium">{productCount}{" "}Products </p>
...
{/* opened menu sorting */}
            <div className={` transition-all overflow-hidden ${open ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
              <div className="flex flex-col  border-l border-b border-r border-chocolate/30 *:hover:bg-chocolate/10 *:text-left *:p-2">
                <button  onClick={() => { onSortChange(null); closePanel(); }}>Featured</button>
                <button  onClick={() => { onSortChange("best-selling"); closePanel(); }}>Best Selling</button>
                <button  onClick={() => { onSortChange("name-asc"); closePanel(); }}>Alphabetically A-Z</button>
                <button onClick={() => { onSortChange("name-desc"); closePanel(); }}>Alphabetically Z-A</button>
                <button  onClick={() => { onSortChange("price-asc"); closePanel(); }}>Price: Low to High</button>
                <button  onClick={() => { onSortChange("price-desc"); closePanel(); }}>Price: High to Low</button>
              </div>
            </div>
          </div>
        </div>
