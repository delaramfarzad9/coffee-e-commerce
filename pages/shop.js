...
export default function Shop({
  ...
  const pushFiltersToUrl = (nextFilters, nextSort) => {
    const q = {
      ...
      inStock:
        nextFilters.inStock === null ? undefined : String(nextFilters.inStock),
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.roast?.length > 0) {
      result = result.filter((item) =>
        filters.roast.includes(item.moreInfo.roast),
      );
    }

    if (filters.process?.length > 0) {
      result = result.filter((item) =>
        filters.process.includes(item.moreInfo.process),
      );
    }

    if (filters.priceFrom !== null)
      result = result.filter((item) => item.price >= filters.priceFrom);
    if (filters.priceTo !== null)
      result = result.filter((item) => item.price <= filters.priceTo);

    if (filters.inStock !== null)
      result = result.filter(
        (item) => Boolean(item.inStock) === Boolean(filters.inStock),
      );
    ...
    // When sorting by best-selling we only want to show true best sellers
    if (sortOption === "best-selling") {
      const { getBestSellerIds } = require("@/utils/bestSellers");
      // getBestSellerIds returns a Set of IDs for top N sellers
  ...
  // Auto-scroll to "Similar Products" block whenever filters produce no results
  useEffect(() => {
    if (filteredProducts.length === 0 && noMatchRef.current) {
      requestAnimationFrame(() => {
        const offset = -80;
        const top =
          noMatchRef.current.getBoundingClientRect().top +
          window.pageYOffset +
          offset;
        window.scrollTo({ top, behavior: "smooth" });
      });
  ...
  const findClosestMatches = (
    ...
    const applyFilters = (products, f) => {
      ...
      if (f.origin?.length > 0) {
        ...
        res = res.filter((item) => {
          const tokens = (item.moreInfo.origin || "")
            .split(/[,&]/)
            .map((s) => s.trim().toLowerCase());
          return wanted.some((w) =>
            tokens.some((t) => t.includes(w) || t === w),
          );
        });
      }

      if (f.roast?.length > 0)
        res = res.filter((item) => f.roast.includes(item.moreInfo.roast));
      if (f.process?.length > 0)
        res = res.filter((item) => f.process.includes(item.moreInfo.process));
      if (f.priceFrom !== null)
        res = res.filter((item) => item.price >= f.priceFrom);
      if (f.priceTo !== null)
        res = res.filter((item) => item.price <= f.priceTo);
      if (f.inStock !== null)
        res = res.filter(
          (item) => Boolean(item.inStock) === Boolean(f.inStock),
        );
    ...
    if (exact.length > 0)
      return { results: exact, reason: "exact", relaxedFilters: activeFilters };
  ...
  const fallback = findClosestMatches(
    products,
    filters,
    sortOption,
    searchQuery,
      ...
      <section className="relative w-full flex flex-col">
        ...
        <div ref={catalogRef}>
          ...
          {filteredProducts.length === 0 && (
            <div
              ref={noMatchRef}
              className="w-full flex flex-col items-center justify-center py-12 gap-4 text-chocolate text-center px-6"
            >
              ...
              <div className="w-full max-w-7xl">
                <Catalog
                  ...
                  title={
                    fallback.reason === "exact" ? "Results" : "Similar Products"
                  }
                  svgId="circle-plus"
